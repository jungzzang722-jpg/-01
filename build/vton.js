...
(function (global) {
  ...
  function compose(body, layers, opts) {
    ...
    return cacheGet(key).then(function (hit) {
      ...
      return toBlob(personCv).then(function (pb) {
        ...
        if (!r.ok) return r;
        /* 모의 결과는 저장하지 않는다. 저장해 버리면 나중에 진짜 모델을 붙여도
         * 같은 옷·같은 사진에서는 계속 모의 결과가 나온다 — 원인을 찾을 수 없는
         * 종류의 버그가 된다. */
        if (r.mock) { r.ms = Math.round((global.performance || Date).now() - t0); return r; }
  ...
  function post(url, fd, stage, signal) {
    ...
    return fetch(url.replace(/\/+$/, '') + '/compose', {
      ...
      if (j.ok && j.image) {
        return { ok: true, dataUrl: j.image, provider: j.provider || null,
                 mock: !!j.mock, quota: j.quota || null };
      }
      return j;
    }).catch(function (e) {
      clearTimeout(timer);
  ...
  function readableError(status, body) {
    /* 서버가 이유를 말해 줬으면 그 말을 쓴다.
     *
     * "합성 서버에 문제가 있습니다(502)" 는 아무것도 알려주지 않는다 —
     * 정작 서버는 무엇이 잘못됐는지 정확히 알고 그 문장을 보내왔는데,
     * 여기서 상태 코드만 보고 지어낸 문장으로 덮어써 버리고 있었다.
     * 사장님이 서버 창을 뒤져야만 원인을 알 수 있는 이유가 이것이었다. */
    var j = null;
    try { j = JSON.parse(body); } catch (e) { }
    if (j && j.ko) return j.ko;
    ...
    run.onclick = function () {
      ...
      VTON.compose(snap.body, snap.layers, {
        ...
        $('vtRemoteCap').textContent = r.cached
          ...
          : r.mock
          ? '⚠ 모의 모드 — 모델이 붙어 있지 않습니다. 인물 사진이 그대로 돌아온 것입니다.'
